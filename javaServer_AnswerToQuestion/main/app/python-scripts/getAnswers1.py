...
def addAnswer(question):
    ...
    try:
        ...
        # 创建一个游标对象
        cursor = db.cursor()
        allStr = "insert into question_and_answer (question_id, question, answers) values('"+question['questionID'].replace('\\', '\\\\').replace("'", "\\'")+"','"+question['questionTitle'].replace('\\', '\\\\').replace("'", "\\'")+"','"+answerStr+"');"
