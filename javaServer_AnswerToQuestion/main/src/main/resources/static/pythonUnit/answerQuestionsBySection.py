...
def answerQuestions2(SbusectionID_now,qustionlst):
    for answertiems in qustionlst:
        ...
        if(result):
            for i in range(2,len(result),2):
                if result[i] and result[i]!='该位置为错误答案' and type(result[i])!=datetime.datetime:
                    strAnswer+=result[i]+','
            strAnswer=strAnswer[:-1]
        else: 
            #数据库中没有答案就返回一个error内容
            strAnswer+='error'
            print('The answer is not available in the database===>',answertiems)
        ...
        print('Successfully answered:',ansreq.json())
        ansreq.close()
    #最后一次返回
    ...
    print('Final request:',ansreq.json())
    ansreq.close()
...
def get_SubID():
        ...
        try:
            ...
            print("sepamrate"+str(QuestionCount)+"separate")
